...
def serialize3DArray(array):
    ...
    return result
displayName = "Schematic2Script #2"
inputs = (
...
def perform(level,box,options):
    ...
    lines.append("    e.root_tag['TileEntities'] = " + str(TAG_List(schema.root_tag['TileEntities'].value)) + ")\n")
    if not ignoreData:
        # copy data array if it's not ignored
        lines.append("    e.root_tag['Data'] = TAG_Byte_Array(" + serialize3DArray(schema.root_tag['Data'].value) + ")\n")
